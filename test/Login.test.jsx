...
describe('LoginForm', () => {
  it('Envía el formulario y redirige en caso de éxito', async () => {
    ...
    mock.onPost('http://localhost:3000/login').reply(200, { message: 'Inicio de sesión exitoso' });
    ...
    await waitFor(() => expect(require('react-router-dom').useNavigate).toHaveBeenCalledTimes(1));
    
    
    expect(require('react-router-dom').useNavigate).toHaveBeenCalledWith('/Menu');
  });
});
